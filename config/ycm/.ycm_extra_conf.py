...
def AddDirsRecursivelyForCppSrcInclude():
  ...
  for flag in flagsRec:
    for d in glob.glob(flag, recursive = True) :
      if os.path.isdir(d):
            new_flags.append('-I')
            new_flags.append(os.path.abspath(d))
  return new_flags
...
flags = flags + AddDirsRecursivelyForCppSrcInclude() #+ GetRosIncludeFlags()
